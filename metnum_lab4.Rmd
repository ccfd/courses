...
\begin{zad} Wydziel z funkcji `Solve` część odpowiedzialną za mnożenie przez $A$: `Mult(double** A, double*x, double* r)` i preconditioner diagonalny: `Precond(double** A, double*r, double* p)` --- Zauważ że mnożenie przez macierz $A$ występuje co najmniej dwa razy w iteracji.\end{zad}

Na tym etapie w funkcji `Solve` nie powinny występować nigdzie elementy macierzy $A$.

\begin{zad} Przenieś zmienne `fix`, `thick` do zmiennych globalnych\end{zad}

\begin{zad}Skopiuj funkcję `Mult` pod nazwą `SMult`\end{zad}
...
\begin{zad}Przekopiuj fragment kodu funkcji `main` odpowiedzialny za konstrukcję macierzy $S$. Następnie, każde wystąpienie
`S[i,j] += cos;`
zamień na:
`r[i] += cos * x[j];`\end{zad}
...
\begin{zad}Zamień pętlę wycinającą $i$ty wiersz, na `r[i]=x[i]`\end{zad}
\begin{zad}Przetestuj kod z `SMult` zamiast `Mult`\end{zad}
\begin{zad}Napisz trywialny preconditioner `IPrecond(double ** A, double * r, double * p)`, przepisujący $p=r$.\end{zad}
\begin{zad}Popraw kod zauważając, że ani `SMult` ani `IPrecond` nie potrzebują brać `A` za argument.\end{zad}
