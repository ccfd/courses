...
- Napisz funkcję `display`, która wyświetli w oknie graficznym aktulne położenia kółek
    - funkcja powina przyjmować te same argumenty co funkcja `init`,
...
Składowe prędkości (`xV` orax `yV`) wylosuj tak, aby ich wartości znalazły się w przedziale $[-1, 1]$.
...
W celu dokonania tego sprawdzenia, dla każdej pary piłek będzie trzeba wyznaczyć odległość i sprawdzić czy jest mniejsza niż $2\cdot R = 40$ (przyjęliśmy wcześniej, że promień piłki to $20$).

Następnie należy zmodyfikować prędkości piłek zgodnie z poniższymi wzorami.

 - $\mathbf{v}'$ to prędkość piłki po zderzeniu.
 - $\Delta \mathbf{v}$ to zmiana predkości.
 - $\mathbf{r}_1$ i $\mathbf{r}_2$ to promienie wodzące piłek (współrzędne).
 - $C$ co to pewna stała, ułatwiająca wyprowadzenie i zapis wzorów.
 
$$\mathbf{v}'_1= \mathbf{v}_1 + \Delta \mathbf{v}$$
$$\mathbf{v}'_2= \mathbf{v}_2 - \Delta \mathbf{v}$$
$$\Delta \mathbf{v} = C \cdot (\mathbf{r}_{1} - \mathbf{r}_{2})$$
gdzie:
