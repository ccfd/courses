...
Kulki zderzając się ze sobą, mogą wymienić się pędem tylko w kierunku zgodnym z prostą przechodzącą przez ich środki.
Przyjmijmy, że kulki mają te same masy. Z równania wynikającego z zas. zachowania pędu wyciągamy wniosek, że prędkości po zderzeniu będą wynosić.
$$
v_{1}'= v_{1}+v_{p}
$$
$$
v_{2}'= v_{2}-v_{p}
$$
gdzie $v_{p}$ to wektor, którego zmiany pędu w postaci:
$$
v_{p}= C*(p_{1}-p_{2})
$$
$$
$p_{1}$ i $p_{2}$ to wektory położenia środków kulek.
$$
Przyjmijmy, że kulki zderzają się sprężyście, zatem możemy zapisać równanie wynikajacę z zas. zachowania energii:
$$
v_{1}^2+v_{2}^2= v_{1}'^2 +v_{2}'^2
$$
Po wstawieniu do tego równania zależności otrzymanych wcześniej otrzymujemy wartość współczynnika C:
$$
C= -(p_{1}-p_{2})o(v_{1}-v_{2})/(|p_{1}-p_{2}|^2)
$$
gdzie iloczyn skalarny ( a właściwie jego znak) informuje nas, czy w ogóle do zderzenia dojdzie; jeśli jest ujemny, to dojdzie.
