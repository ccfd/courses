...
- Napisz funkcję `initPositions`, która losuje położenia początkowe kółek tak, aby mieściły się w oknie graficznym.
Użyj funkcji `rand()` z biblioteki `stdlib.h` (patrz zajęcia $4$).
- Napisz funkcję `display`, która wyświetli w oknie graficznym aktualne położenia kółek
    - funkcja powinna przyjmować te same argumenty co funkcja `init`,
    - przyjmij, że promienie kółek są równe $R = 20$.
...
Piłeczki powinny poruszać się, musimy zatem:

- określić ich prędkości początkowe oraz
- ustalić prawo ruchu piłeczek.
...
W celu dokonania tego sprawdzenia, dla każdej pary piłek należy wyznaczyć odległość i sprawdzić czy jest mniejsza niż $2\cdot R = 40$ (przyjęliśmy wcześniej, że promień piłki to $R = 20$).
...
Oznaczenia:
 ...
 - $C$ -- stała, ułatwiająca wyprowadzenie i zapis wzorów.
...
1. Z poczynionych założeń (1. i 2.) wynika, że w momencie zderzenia na kulki będzie działać siła o kierunku takim samym jak wektor łączący środki kulek.
...
$A$ to pewna stała, która nie zmieni kierunku wektora ale zmieni jego zwrot i wartość.
...
rola stałej $B$ jest taka sama jak stałej $A$.
...
3. W tym momencie mamy $7$ niewiadomych ($\mathbf{p}'_1$, $\mathbf{p}'_2$, $\Delta \mathbf{p}$, B) i $6$ równań ($3$ r-nia wektorowe).
Musimy skorzystać także z równania zachowania energii:
$$
...
Otrzymaną stałą możemy podstawić do równania (1), które z kolei podstawiamy do równania (2) otrzymując szukane wyrażenia na prędkości po zderzeniu.
