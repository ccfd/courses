---
title: "Metody Numeryczne - Skrypt 0"
...
Taki wektor nazywamy residualem. Zazwyczaj by go zmierzyć jedną liczbą mówimy że residual to $\|r\|_2=\sqrt{\sum_ir_i^2}$. Aktualnie wynosi on `r sqrt(sum(r))`.
...
draw.residuals = function(name) {
  if (!missing(name)) resid = resid[resid$name %in% name,]
  resid$name = factor(resid$name)
