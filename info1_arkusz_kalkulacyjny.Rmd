...
Po zaznaczeniu komórki, w jej prawym dolnym rogu pojawi się mały kwadracik -- wystarczy kliknąć w niego i trzymając klawisz przeciągnąć pionowo lub poziomo.
...
- Kolumnę `Imię i Nazwisko` należy wypełnić korzystając z funkcji `CONCATENATE()` i `VLOOKUP()`. Funkcje można zagnieżdżać, np. wywołanie `VLOOKUP(CONCATENATE(...), CONCATENATE(...))` jest prawidłowe.  
...
- Dodaj obiekt typu formuła w celu zapisania równania.
...
- Wyznacz wartości miejsc zerowych funkcji (formuła `SQRT()` zwraca pierwiastek kwadratowy z liczby).
Uwzględnij przypadek, w którym pierwiastek nie istnieje (niech w takim przypadku w komórce znajdzie się napis ,,brak'').

![Rys.6. Miejsca zerowe funkcji kwadratowej.](figures/info1/sheet_5.png "Miejsca zerowe funkcji kwadratowej.")

## Importowanie danych z pliku
Ściągnij przykładowy plik [wyniki.dat](data/info1/dane.dat) i przenieś do katalogu z plikiem arkusza kalkulacyjnego.
Dane można ściągnąć klikając *File* -> *Open* a następnie wybierając plik `wyniki.dat`.
Otworzy się nowe okno, które pozwala wybrać opcje importu, min. sekcja *Separator Options* pozwala wybrać w jaki sposób rozdzielone są dane (zaznaczamy *Space*).
Zaimportowane dane często wymagają zmiany separatora dziesiętnego.
Klikamy *Edit* -> *Find and replace*, następnie w polu *Find* piszemy przecinek a w polu *Replace* kropkę i klikamy *Replace All*.
...
![Rys.5. Wykres funkcji dwóch zmiennych.](figures/info1/excel.png "Wykres funkcji dwóch zmiennych.")
