...
We made the experiment by placing the tank at several different heights (eg 2, 3 and 4 m) and for each of these heights we measured the time after which the tank was empty. Note, that pipe remains on the same level. 
...
The uniform division of an resulting arc (measuring in local arc coordinate) on $n - 1$ allows
to generate abscisas of the interpolation nodes given be the formula:
$$
